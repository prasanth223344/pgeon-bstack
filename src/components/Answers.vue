    ...
    <main class="landing-main mw6 m-auto pl15 pr15 smtp mt15p">
      ...
      <div class="open-question__responses">
        <div v-for="answer in answers">
          ...
          <div
            class="open-question__response"
            v-bind:class="{ 'fadeIn':  answer.attrs._id == pushed_id}"
            v-if="ownerOfAnswer(answer.user_id)"
          >
            <p>{{answer.answer}}</p>
      ...
      <div v-else>
       <div v-for="(answer, index) in answers"  v-bind:key="answer.attrs._id">
        ...
        <use v-bind:class="{ 'vote-up': checkVoted(answer.attrs._id) == 1 }"
                  class="caret-up" :xlink:href="caretup"></use>
              ...
              >{{(answer.vote_count)?answer.vote_count:0 }}</div>
...
export default {
 ...
 computed: {
    caretup() {
      const sprite = require("../assets/img/sprites/solid.svg"); 
      return sprite + '#caret-up';
    },
    caretdown() {
      const sprite = require("../assets/img/sprites/solid.svg"); 
      return sprite + '#caret-down';
    }
  },
  watch: {
    answers() {
...
//console.log(this.RADIKS_SERVER);
    ...
    delete_answer: function(id) {
      ...
      this.$http.delete("/answer/" + id, formData).then(
        response => {
          this.submitted_text = "";
          location.reload();
          // this.fetchRecords()
    ...
    castVote: function(answer_id, vote) {
      ...
      this.$http.post("/vote", formData).then(
        response => {
    ...
    getVoteCount() {
      var com = this;
      $.getJSON(
        "/get_vote_count_for_question/" + this.question_id,
        function(votes) {
