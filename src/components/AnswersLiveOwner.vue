<template>
  <div>
    ...
    <main
      class="landing-main mw6 m-auto pl15 pr15 smtp mt15p"
      v-bind:class="[{ 'confirming-modal--active':  showendmodal == true }]"
    >
        ...
        <div class="open-question__meta">
          ...
          <div>
            ...
            <span class="open-question__time">
              <answeringtimer :initial="parseInt(question.expiring_at)"></answeringtimer>
            </span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    endnow: async function() {
      ...
      qModel.update({
        last_event_fired: "question_ended",
        expiring_at: moment().unix()
      });
    ...
    cancelnow: function() {
      this.$http.delete(`/question/${this.question_id}`).then(
        response => {
          socket.emit("cancel_now", this.question_id);
          setTimeout(function() {
            location.href = "/my-questions";
          }, 1000);
        },
        response => {
          alert("error submitting");
